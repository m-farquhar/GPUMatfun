		 ...
		 )
{
    ...
    if (nrhs != 1) {
        mexErrMsgIdAndTxt( "MATLAB:get_U:invalidNumInputs",
                "Handle input argument required.");
    }

    if(nlhs != 1){
        mexErrMsgIdAndTxt( "MATLAB:get_U:maxlhs",
    ...
    plhs[0] = mxCreateDoubleMatrix(lanczos_solver_p->Usol.rows(), lanczos_solver_p->Usol.cols(), mxREAL);

    // Copy over the values
    lanczos_solver_p->get_U(mxGetPr(plhs[0]));

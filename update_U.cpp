		 ...
		 )
{
    ...
    if (nrhs != 1) {
        mexErrMsgIdAndTxt( "MATLAB:update_U:invalidNumInputs",
                "Handle input argument required.");
    }

    if(nlhs != 0){
        mexErrMsgIdAndTxt( "MATLAB:update_U:maxlhs",
                "No output arguments are required.");
    }

    /* Check data type of input arguments  */
    if (!(mxIsUint64(prhs[0]))){
        mexErrMsgIdAndTxt( "MATLAB:update_U:inputNotHandle",
